 ...
 *
 */
Spawner.prototype.start = function(meta, uid, cb) {

	var self = this

	var spawn = this.spawn(meta, uid)
	...
	function onExit(err) {
		self._remove(spawn, meta.user.username, spawn.uid, function(err, result) {
			if (err) {
	...
	spawn.once('build::start', onStart)
	spawn.once('build::exit', onExit)
 ...
 *
 */
Spawner.prototype.cleanAll = function(cb) {
	var self = this
	this.stopAll(function() {
		raft.common.rmApps(raft.directories.package, function() {
			raft.common.rmApps(raft.directories.tmp, cb)
		})
	})
};
/**

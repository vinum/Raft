...
// 5 seconds
var HEARTBEAT_PERIOD = 20 * 1000;
// 20 seconds
var HEARTBEAT_FAILS = 3;
...
var LogHarvester = function(conf) {
	...
	if (!conf.server) {
		throw Error("Missing server configuration");
	}
	if (!conf.log_file_paths) {
		conf.log_file_paths = {}
	};
...
LogHarvester.prototype = {
	...
	run : function() {
		...
		setInterval(function() {
			harvester._log.info("Watching " + __(harvester.log_files).size() + " log files, " + " sent " + harvester.messages_sent + " log messages.");

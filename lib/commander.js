...
Command.prototype.command = function(name) {
	var args = name.split(/ +/);
	var cmd = new Command(args.shift(), this.logName);
...
Command.prototype.missingArgument = function(name) {
	raft.log.error(this.logName);
	raft.log.error(this.logName, "  error: missing required argument `%s'", name);
	raft.log.error(this.logName);

	raft.log.info(this.logName, 'Raft NOT ok!'.red + '"')
	process.exit(1);
};
...
Command.prototype.optionMissingArgument = function(option, flag) {
	raft.log.error(this.logName);
	if (flag) {
		raft.log.error(this.logName, "  error: option `%s' argument missing, got `%s'", option.flags, flag);
	} else {
		raft.log.error(this.logName, "  error: option `%s' argument missing", option.flags);
	}
	raft.log.error(this.logName);
...
Command.prototype.unknownOption = function(flag) {
	raft.log.error(this.logName);
	raft.log.error(this.logName, "  error: unknown option `%s'", flag);
	raft.log.error(this.logName);

	raft.log.info(this.logName, 'Raft NOT ok!'.green + '"')
	process.exit(1);
};
...
Command.prototype.version = function(str, flags) {
	...
	this.on('version', function() {
		raft.log.info(this.logName, 'Version Number: ' + str);

		raft.log.info(this.logName, 'Raft ok!'.green + '"')
		process.exit(0);
	});
	return this;
};
...
Command.prototype.promptMultiLine = function(str, fn) {
	var buf = [];
	raft.log.info(this.logName, str);
	process.stdin.setEncoding('utf8');
...
Command.prototype.password = function(str, mask, fn) {
	...
	process.stdin.on('keypress', function(c, key) {
		...
		if (key && key.ctrl && 'c' == key.name) {
			raft.log.info(this.logName, '%s', buf);
...
Command.prototype.choose = function(list, index, fn) {
	...
	list.forEach(function(item, i) {
		if (hasDefault && i == index) {
			raft.log.info(this.logName, '* %d) %s', i + 1, item);
		} else {
			raft.log.info(this.logName, '  %d) %s', i + 1, item);
		}
	});
...
function outputHelpIfNecessary(cmd, options) {
	...
	for (var i = 0; i < options.length; i++) {
		if (options[i] == '--help' || options[i] == '-h') {
			raft.log.info(cmd.logName, cmd.helpInformation());
			cmd.emit('--help');

			raft.log.info(cmd.logName, 'Raft ok!'.green + '"')
			process.exit(0);
		}
	}
}
...
var crypto = require('crypto'), fs = require('fs'), http = require('http'), os = require('os'), path = require('path'), exec = require('child_process').exec, spawn = require('child_process').spawn, flatiron = require('flatiron'), async = flatiron.common.async, rimraf = flatiron.common.rimraf, haibu = require('../../haibu');
...
common.showWelcome = function(role, ipAddress, port) {
	...
	var plugins = Object.keys(haibu.plugins), serverMsg;
	...
	if (Array.isArray(role)) {
		for (var i = 0; i < role.length; i++) {
			serverMsg = ['haibu'.yellow.bold, 'started @'.grey, (role[i][1] || '127.0.0.1').green.bold, 'on port'.grey, role[i][2].toString().green.bold, 'as'.grey, role[i][0].green.bold].join(' ');

			console.log('  ' + serverMsg);
		};
	} else {
		serverMsg = ['haibu'.yellow.bold, 'started @'.grey, (ipAddress || '127.0.0.1').green.bold, 'on port'.grey, port.toString().green.bold, 'as'.grey, role.green.bold].join(' ');
...
// family is found return the loopback addres `127.0.0.1`.
//
common.registerHive = function(callback) {
	var address = haibu.server.address()
	haibu.server.hash = common.hash(address.address, address.address)

	function create() {
		new haibu.mongoose.Hive({
			domain : os.hostname() + '.hive.' + (haibu.config.get('proxy:domain') || 'localhost'),
			port : address.port,
			host : address.address,
			hash : haibu.server.hash,
			online : true
		}).save(callback)
	}


	haibu.mongoose.Hive.findOne({
		hash : haibu.server.hash,
	}).run(function(err, hive) {
		if (err)
			throw err
		if (!hive) {
			create()
			...
			hive.online = true
			hive.port = address.port
			hive.host = address.address
			hive.domain = os.hostname() + '.hive.' + (haibu.config.get('proxy:domain') || 'localhost')
			hive.save(callback)
		}
	})
};
...
// family is found return the loopback addres `127.0.0.1`.
//
common.unregisterHive = function(callback) {
	haibu.mongoose.Hive.findOne({
		hash : haibu.server.hash,
	}).run(function(err, hive) {
		if (err)

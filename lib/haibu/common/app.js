...
var FAKE = {
	ctime : 1351370845992,
	command : 'node',
	file : '/home/bob/Documents/node/Raft/node_modules/haibu-carapace/bin/carapace',
	foreverPid : 9819,
...
var Drone = function(info, packageHash) {
	...
	this.hive = haibu.api.hives[this.info.hiveHash]
...
App.prototype.init = function(package, callback) {
	...
	haibu.mongoose.Package.findOne({
		name : this.appname,
		user : this.username,
		status : true
		...
		if (oldPackage) {
			...
			oldPackage.save(function() {
				...
				new haibu.mongoose.Package(package).save(onPackageSave)
			})
		} else {
...
App.prototype._getHive = function(callback) {
	...
	haibu.mongoose.Hive.findOne({
		...
		var cli
		if (!( cli = haibu.api.hives[hive.hash])) {
			cli = haibu.api.hives[hive.hash] = new haibu.clients.Drone(hive)
		}
...
App.prototype._getCli = function(hash) {
	var self = this
	return haibu.api.hives[hash]
}

App.prototype._getSnapShot = function(callback) {
	var self = this
	console.log(this.version)
	haibu.mongoose.SnapShot.findOne({

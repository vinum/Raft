...
var fs = require('fs'), path = require('path'), forever = require('forever-monitor'), semver = require('semver'), mixin = require('flatiron').common.mixin, haibu = require('../../haibu');

haibu.getSpawnOptions = function getSpawnOptions(app) {
	var engine = (app.engines || app.engine || {
		node : app.engine
	}).node, engineDir = haibu.config.get('directories:node-installs'), nodeVersions = engineDir && fs.readdirSync(engineDir), carapaceDir = haibu.config.get('directories:carapace-installs'), carapaceVersions = carapaceDir && fs.readdirSync(carapaceDir), env = {}, command = 'node', nodeDir, version, cwd;
	...
	//options.carapaceBin = path.join(__dirname, '..', '..', '..', '..', 'haibu-carapace', 'bin', 'carapace.js');
	if (version) {
		//
	...
	fs.stat(repo.startScript, function(err, stats) {
		...
		function onCarapacePort(info) {
			if (!responded && info && info.event === 'port') {
				...
				haibu.common.bcryptHash(haibu.common.uuid(), function(err, hash) {

...
exports.createRouter = function(drone) {
	...
	raft.router.path('/user', function() {
		...
		this.post('/:username', function(username) {
			...
			var newUserCoupon = new raft.mongoose.Coupon({
				userid : newUser._id,
			...
			newUser.save(function(err) {
				...
				newUserProfile.save(function(err) {
					...
					newUserCoupon.save(function(err) {
						if (err) {
							return raft.error(err, res)
						}
						raft.mail.send({
							from : '"Noreply - MangoRaft" <' + raft.config.get('mail:auth:user') + '>',
							to : '"' + username + '" <' + email + '>',
							subject : 'Confirm account',
							text : 'http://api.' + raft.config.get('domain') + '/user/' + newUserCoupon.code + '/confirm',
							html : 'http://api.' + raft.config.get('domain') + '/user/' + newUserCoupon.code + '/confirm',
			...
			raft.mongoose.Coupon.findOne({
				...
				if (coupon.confirmed) {
					...
					raft.mongoose.User.findOne({
						...
						user.confirmed = coupon.confirmed = true
